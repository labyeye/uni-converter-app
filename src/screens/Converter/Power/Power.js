...
const Power = () => {
  ...
  const handleButtonPress = (symbol) => {
    if (symbol === 'C') {
      ...
      setInput(newInput);
      setConvertedValue(convertPower(newInput, fromUnit, toUnit));
    } else {
  ...
  const convertPower = (value, fromUnit, toUnit) => {
    ...
    const valueInJoules = parseFloat(value) / conversionRates[fromUnit];
    const convertedValue = (valueInJoules * conversionRates[toUnit]).toFixed(4);
  ...
  const handleReverse = () => {
    ...
    setFromUnit(toUnit);
    setToUnit(tempUnit);
    const newConvertedValue = convertPower(input, toUnit, fromUnit);
            ...
            {showFromPicker && (
              <Picker
                ...
                onValueChange={(itemValue) => {
                  ...
                  setConvertedValue(convertPower(input, itemValue, toUnit));
                  setShowFromPicker(false);
                }}>
                <Picker.Item label="Watt" value="W" />
                <Picker.Item label="Kilo Watt" value="kW" />
                <Picker.Item label="Horse Power" value="hP" />
                {/* Add more units here */}
              </Picker>
            )}
            <TextInput
              ...
              onChangeText={(text) => {
                setInput(text);
                setConvertedValue(convertPower(text, fromUnit, toUnit));
              }}
          ...
          <View style={styles.inputRow}>
            ...
            {showToPicker && (
              <Picker
                ...
                onValueChange={(itemValue) => {
                  setToUnit(itemValue);
                  setConvertedValue(convertPower(input, fromUnit, itemValue));
